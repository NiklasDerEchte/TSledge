...
- SocketIO handling
- Irgendwie kann man das repl script unter `bin/repl.ts` an Projekte weiterreichen wenn die TSledge installieren.
  - In der `package.json` wurde dazu eine `bin` option erstellt
  - Ich glaube ich brauche noch irgend ein Shebang wie `#!/usr/bin/env node` in der `bin/repl.ts`. Bin aber nicht sicher
